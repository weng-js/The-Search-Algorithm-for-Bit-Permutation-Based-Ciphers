...
void FileOutputTrail() {
	...
	for (int r = 0; r < Rnum; r++) {
		...
		for (int k = 0xf; k >= 0; k--) {
			message << hex << BestTrail[r].m128i_u8[k];
		}
		message << "\nSO[" << dec << setw(2) << setfill('0') << r + 1 << "]: 0x";
		for (int k = 0xf; k >= 0; k--) {
			message << hex << SO[r].m128i_u8[k];
		}
			...
			if (s.j == s.sbx_num) {
				...
				if (s.rnum + 1 == Rnum) {
					...
					if (nxt_s.rnum >= 4 && nxt_s.rnum <= (Rnum - 4)) {
						...
						if (asn == 2) {						
							auto itorYes = ValueMapCluster.find(make_pair(make_pair(make_pair(nxt_s.sbx_a[0], nxt_s.sbx_in[0]), make_pair(nxt_s.sbx_a[1], nxt_s.sbx_in[1])), nxt_s.rnum));
							if (itorYes == ValueMapCluster.end() || itorYes->second.SearchTag(nxt_s.W)) {
								...
								if (Tmp_RecordTrailNUM != RecordTrailNUM) {
									if (itorYes == ValueMapCluster.end()) {
										...
										ValueMapCluster.insert(make_pair(make_pair(make_pair(make_pair(nxt_s.sbx_a[0], nxt_s.sbx_in[0]), make_pair(nxt_s.sbx_a[1], nxt_s.sbx_in[1])), nxt_s.rnum), NewValueForCluster));
										itorYes = ValueMapCluster.find(make_pair(make_pair(make_pair(nxt_s.sbx_a[0], nxt_s.sbx_in[0]), make_pair(nxt_s.sbx_a[1], nxt_s.sbx_in[1])), nxt_s.rnum));
									}
									else {
...
void matsui() {
	...
	// input info:
	Rnum = 14;                  // the number of rounds for distinguisher
	...
	//TrailIn.m128i_u8[0] = 0x7;
	//TrailIn.m128i_u8[12] = 0x7;
	//TrailOut.m128i_u8[2] = 0xc;
	//TrailOut.m128i_u8[10] = 0xc;
#endif
